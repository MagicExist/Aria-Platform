...
|----|-------------|--------|-------------------|
| TC-1 | La app es **solo Android** (React Native/Expo) | intent-statement + P1 + gate feasibility | Sin web ni iOS; target Android único |
| TC-2 | Stack: **React + Expo + TypeScript** | P1, P2 | Lenguaje y tooling fijados; librerías del ecosistema RN/Expo |
| TC-3 | **Web e iOS quedan fuera de alcance** (iOS no verificable sin Mac; web no requerida) | gate feasibility 2026-07-10 | Elimina el riesgo de iOS; una sola plataforma que probar |
...
| TC-8 | Distribución del MVP = **Android APK** (sideload), sin tiendas | P5 | Empaquetado APK (EAS/local); única plataforma objetivo |
