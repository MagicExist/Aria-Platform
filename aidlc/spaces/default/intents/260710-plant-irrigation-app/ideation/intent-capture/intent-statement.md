...
aplicación multiplataforma (web, Android e iOS) conectada a datos de sensores
(IoT) es un objetivo explícito en sí mismo, no solo un medio.
...
- **Beneficio:** un tablero único, accesible desde web y móvil, que muestra las
  variables de cada planta, las agrupa por zona de riego y notifica anomalías.
...
4. **Acceso multiplataforma:** un usuario puede crear cuenta, iniciar sesión y
   usar la app en web y Android (iOS se compila pero **no se verificará
   localmente** por falta de hardware Mac).
5. **Aprendizaje (meta cualitativa):** una app multiplataforma funcional que
   consume datos de un backend de sensores, desplegable/ejecutable de extremo a
   extremo.
...
quiere construir un sistema multiplataforma conectado a sensores IoT. Existe (o
llegará) hardware de sensores para plantas y falta el software que reciba,
centralice y visualice esos datos. No hay presión de mercado ni regulatoria; la
...
- Multiplataforma: web, Android e iOS (iOS sin verificación local).
